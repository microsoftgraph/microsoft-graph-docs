...
Collect the platform-specific application IDs for each application and platform that will use [Project Rome APIs](https://developer.microsoft.com/graph/do(/graph/api/resources/project_rome_overview?view=graph-rest-1.0).
...
If you have an app using the activity feed API in Microsoft Graph, your host value must be supplied in the **activitySourceHost** property. For details, see [activity resource type](https://developer.microsoft.com/graph/do(/graph/api/resources/projectrome_activity?view=graph-rest-1.0).

...
|servicePrincipalType|String|Identifies if the service principal represents an application, a managed identity, or a legacy application. This is set by Azure AD internally. The **servicePrincipalType** property can be set to three different values: <ul><li>__Application__ - a service principal that represents an [application](./application.md). The **appId** property *must* be set the unique identifier for the associated app registration. If that app registration is missing, tokens are not issued for the service principal.</li><li>__ManagedIdentity__ - a service principal that represents a [managed identity](/azure/active-directory/managed-identities-azure-resources/overview).</li><li>__Legacy__ - a service principal that represents an app created before app registrations. The service principal can have credentials, service principal names, reply URLs, and other properties which are editable by the user, but does not have a backing application. The **AppId** exists but doesn't point to anything. The service principal only works in the tenant where it was created.</li></ul>|

...
|tags|String collection| Custom strings that can be used to categorize and identify the service principal. Not nullable. Possible value that are non-exhaustive are <ul><li> 'WindowsAzureActiveDirectoryIntegratedApp' is used to have this service principal show up in the list of Integrated Applicatins in the Admin Portal </li><li> 'HideApp' is used hide the application for users. This is similar to functionailty enabled by the setting "Visible to users" in the properties section for Enterprise Applications. [Updating tags using Powershell](/azure/active-directory/manage-apps/hide-application-from-user-portal) </li></ul>|

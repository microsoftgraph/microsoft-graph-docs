...
Get user detail about Skype for Business device usage.
...
```http
GET /reports/SkypeForBusinessDeviceUsageUserDetail(period='{period_value}')
GET /reports/SkypeForBusinessDeviceUsageUserDetail(date={date_value})
```
...
```http
GET https://graph.microsoft.com/v1.0/reports/SkypeForBusinessDeviceUsageUserDetail(period='D7')
```
...
```http
HTTP/1.1 200 OK
Report Refresh Date,User Principal Name,Last Activity Date,Windows,Windows Phone,Android Phone,iPhone,iPad,Report Period
```

...
```http
GET /reports/SharePointSiteUsageStorage(period='{period_value}')?$format=application/json
```
...
If successful, this method returns a `200 OK` response code and a **siteUsageStorage** object in the response body.
...
<!-- {
  "blockType": "request",
  "name": "reportroot_sharepointsiteusagestorage"
}-->

```http
GET https://graph.microsoft.com/beta/reports/SharePointSiteUsageStorage(period='D7')?$format=application/json
```

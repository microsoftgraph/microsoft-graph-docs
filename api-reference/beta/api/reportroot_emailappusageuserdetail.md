...
```http
GET /reports/EmailAppUsageUserDetail(period='{period_value}')?$format=application/json
GET /reports/EmailAppUsageUserDetail(date={date_value})?$format=application/json
```
...
<!-- {
  "blockType": "request",
  "name": "reportroot_emailappusageuserdetail"
}-->

```http
GET https://graph.microsoft.com/beta/reports/EmailAppUsageUserDetail(period='D7')?$format=application/json
```

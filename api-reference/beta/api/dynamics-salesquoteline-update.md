...
```
PATCH /financials/companies('{id}')/salesQuotes('{id}')/salesQuoteLines('{salesQouteLineId}')
```
...
```json
PATCH https://graph.microsoft.com/beta/financials/companies('{id}')/salesQuotes('{id}')/salesQuoteLines('{salesQuoteLineId}')
Content-type: application/json

...
```http
GET /reports/OneDriveActivityFileCounts(period='{period_value}')?$format=application/json
```
...
<!-- {
  "blockType": "request",
  "name": "reportroot_onedriveactivityfilecounts"
}-->

```http
GET https://graph.microsoft.com/beta/reports/OneDriveActivityFileCounts(period='D7')?$format=application/json
```

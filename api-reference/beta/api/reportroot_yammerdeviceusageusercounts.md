...
```http
GET /reports/YammerDeviceUsageUserCounts(period='{period_value}')?$format=application/json
```
...
<!-- {
  "blockType": "request",
  "name": "reportroot_yammerdeviceusageusercounts"
}-->

```http
GET https://graph.microsoft.com/beta/reports/YammerDeviceUsageUserCounts(period='D7')?$format=application/json
```

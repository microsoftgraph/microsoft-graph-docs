...
```http
GET /reports/SharePointActivityFileCounts(period='{period_value}')?$format=application/json
```
...
If successful, this method returns a `200 OK` response code and a **siteActivitySummary** object in the response body.
...
<!-- {
  "blockType": "request",
  "name": "reportroot_sharepointactivityfilecounts"
}-->

```http
GET https://graph.microsoft.com/beta/reports/SharePointActivityFileCounts(period='D7')?$format=application/json
```

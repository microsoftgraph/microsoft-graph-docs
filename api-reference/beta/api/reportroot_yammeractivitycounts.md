...
```http
GET /reports/YammerActivityCounts(period='{period_value}')?$format=application/json
```
...
If successful, this method returns a `200 OK` response code and a **yammerActivitySummary** object in the response body.
...
<!-- {
  "blockType": "request",
  "name": "reportroot_yammeractivitycounts"
}-->

```http
GET https://graph.microsoft.com/beta/reports/YammerActivityCounts(period='D7')?$format=application/json
```

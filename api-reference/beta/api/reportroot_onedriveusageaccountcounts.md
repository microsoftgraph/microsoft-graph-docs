...
Get the trend in the number of active OneDrive for Business sites. Any site in which users had viewed, modified, uploaded, downloaded, shared, or synced files are considered sites with file activity and are considered active sites.
...
```http
GET /reports/OneDriveUsageAccountCounts(period='{period_value}')?$format=application/json
```
...
If successful, this method returns a `200 OK` response code and an **oneDriveUsageAccountCounts** object in the response body.

The **oneDriveUsageAccountCounts** object has the following properties.
...
```http
GET https://graph.microsoft.com/beta/reports/OneDriveUsageAccountCounts(period='D7')?$format=application/json
```
